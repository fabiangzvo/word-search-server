...
interface Metadata { section?: string }

export class Logger {
    private static instance: WinstonLogger;
    private static formatLog = printf(({ level, message, timestamp, metadata }) => {
    ...
    public static getInstance(): WinstonLogger {
        if (!Logger.instance) {
            Logger.instance = createLogger({
                level: 'info',
                format: combine(timestamp({ format: "YYYY-MM-DD HH:MM:SS" }), colorize(), metadata({ fillWith: ['section'] }), this.formatLog),
